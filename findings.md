...
- Windows 路线当前的验收关键点不是“已经把 Tauri 包完整打出来”，而是：
  - 浏览器端正式入口可用于高频验证；
  - Windows 本地启动器可直接指向同一路由和同一服务契约；
  - 当前仍保持 `Tauri` 为后续阶段性打包复验优先壳方案。
- 由于服务测试与浏览器点测都采用正式 MySQL 保留数据模式，独立验收执行后本地数据快照会继续增长；因此 `本地MySQL数据库信息.md` 中的留存数据快照必须同步更新，而不能沿用上一次回归的数字。
- 当前 `P1` 独立验收结论已经成立：
  - `P1验收级专项=15 passed`
