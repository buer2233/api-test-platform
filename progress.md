...
- 为 `platform_core` 新增 `existing_api_asset` 专项规则校验，并让旧接口检查结果输出 `validation_status` / `validation_errors`，同时在无效快照时阻断导出。
...
- `python -m pytest tests/platform_core/test_services_and_assets.py -k "legacy_public_api_catalog" -v`
  - 结果：`4 passed`
- `python -m pytest tests/platform_core/test_templates_and_rules.py -k "existing_api" -v`
  - 结果：`2 passed`
- `python -m pytest tests/platform_core/test_services_and_assets.py -k "legacy_public_api_catalog" -v`
  - 结果：`5 passed`
